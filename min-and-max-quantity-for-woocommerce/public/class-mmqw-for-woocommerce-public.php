...
class MMQW_Min_Max_Quantity_For_WooCommerce_Public {
                ...
                if ( 'on' === $mmqw_group_status ) {
	                ...
	                if ( is_array( $mmqw_rules ) ) {
                        foreach( $mmqw_rules as $mmqw_rule ) {
							...
							if ( 'product' === $rule_condition ) {
								if ( 'is_equal_to' === $rule_condition_is ) {
									if ( $group_min_qty >= 0 && in_array( $current_product_id, $mmqw_rule_values, true ) ) {
										$args['min_value'] = intval($group_min_qty); // Starting value
										...
										if ( isset( $group_max_qty ) && $group_min_qty <= $group_max_qty ) {
											$args['max_value'] = $group_max_qty; // Ending value
										}
									}	
								} else {
									if ( $group_min_qty >= 0 && !in_array( $current_product_id, $mmqw_rule_values, true ) ) {
							...
							$args = $this->mmqw_apply_variation_rules( $sm_post_id, $args, $product, $rule_condition, $rule_condition_is, $mmqw_rule_values, $group_min_qty, $group_max_qty );
							/** Quantity on Variable product section end here */

							/** Quantity on Category section start here */
							$args = $this->mmqw_apply_category_rules( $sm_post_id, $args, $product, $rule_condition, $rule_condition_is, $mmqw_rule_values, $group_min_qty, $group_max_qty );
	 ...
	 * @return array
	 */
	private function mmqw_apply_category_rules( $sm_post_id, $args, $product, $rule_condition, $rule_condition_is, $rule_condition_value, $group_min_qty, $group_max_qty ) {
		if ( 'category' === $rule_condition ) {
			$product_id = $product->get_id();
			if ( !is_product() && ('variable' === $product->get_type() || 'variation' === $product->get_type()) ) {
				$product_id = $product->get_parent_id();
			}
			$all_product_cate_id_list = wp_get_post_terms( $product_id, 'product_cat', array( 'fields' => 'ids' ) );
				...
				if ( 'is_equal_to' === $rule_condition_is ) {
					if ( $min_qty >= 0 && $check_cat_count > 0 ) {
						...
						if ( isset( $max_qty ) && $min_qty <= $max_qty ) {
							$args['max_qty'] = $max_qty; // Ending value
							$args['max_value'] = $max_qty; // Ending value
						}
					}
				} else {
					if ( $min_qty >= 0 && $check_cat_count <= 0 ) {
	 ...
	 * @return boolean
	 */
	private function mmqw_add_to_cart_category_validation( $sm_post_id, $product_id, $quantity, $group_min_qty, $group_max_qty, $rule_condition, $rule_condition_is, $rule_condition_value, $rule_passed ) {
	 ...
	 * @return mixed
	 */
	private function mmqw_apply_variation_rules( $sm_post_id, $args, $product, $rule_condition, $rule_condition_is, $rule_condition_value, $group_min_qty, $group_max_qty ) {
		...
		if ( 'variable_product' === $rule_condition ) {
			if ( 'is_equal_to' === $rule_condition_is ) {
				...
				if ( $min_qty >= 0 && in_array( $current_variation_id, $variation_ids, true ) ) {
					...
					if ( isset( $max_qty ) && $min_qty <= $max_qty ) {
						$args['max_qty']   = $max_qty; // Ending value
						$args['max_value'] = $max_qty; // Starting value
					}
				}
			} else {
				/** Check the min and max value for each product variation and set accordingly */
		...
		/** Quantity on Category section start here */
		$args = $this->mmqw_apply_category_rules( $sm_post_id, $args, $product, $rule_condition, $rule_condition_is, $rule_condition_value, $group_min_qty, $group_max_qty );
		/** Quantity on Category section end here */
	 ...
	 */
	public function mmqw_add_to_cart_validation( $passed, $product_id, $quantity, $variation_id = null ) {
		...
		if ( isset( $get_all_sm ) && ! empty( $get_all_sm ) ) {
			foreach ( $get_all_sm as $rule ) {
				...
				if ( $post_id_language_code === $default_lang ) {
					if ( function_exists( 'icl_object_id' ) ) {
						$current_product_id = icl_object_id( $product_id, 'product', true, $default_lang );
					} else {
				                ...
				                if ( is_array( $mmqw_rules ) ) {
			                        foreach( $mmqw_rules as $mmqw_rule ) {
										...
										if( "simple" === $product_type ) {
											...
											$passed = $this->mmqw_add_to_cart_category_validation($sm_post_id, $current_product_id, $quantity, $group_min_qty, $group_max_qty, $rule_condition, $rule_condition_is, $mmqw_rule_values, $passed);
		...
		if( !$passed ){
			wc_add_notice( __( 'Sorry! you reached maximum limit for this product!', 'min-and-max-quantity-for-woocommerce' ), 'error' );
		}
