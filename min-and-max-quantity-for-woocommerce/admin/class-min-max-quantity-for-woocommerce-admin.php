...
if ( !class_exists( 'MMQW_Min_Max_Quantity_For_WooCommerce_Admin' ) ) {
    class MMQW_Min_Max_Quantity_For_WooCommerce_Admin {
        ...
        public function mmqw_enqueue_scripts( $hook ) {
            ...
            if ( false !== strpos( $hook, '_page_mmqw' ) ) {
                ...
                $select2_handle = ( wp_script_is( 'wc-select2', 'registered' ) ? 'wc-select2' : 'select2' );
                wp_enqueue_script(
                ...
                wp_localize_script( $this->plugin_name, 'coditional_vars', array(
                    ...
                    'qty_on_regular_price'             => esc_html__( 'Regular Price', 'min-and-max-quantity-for-woocommerce' ),
                    ...
                    'qty_on_length'                    => esc_html__( 'Length', 'min-and-max-quantity-for-woocommerce' ),
                    'qty_on_width'                     => esc_html__( 'Width', 'min-and-max-quantity-for-woocommerce' ),
                    'qty_on_height'                    => esc_html__( 'Height', 'min-and-max-quantity-for-woocommerce' ),
                    'qty_on_weight'                    => esc_html__( 'Weight', 'min-and-max-quantity-for-woocommerce' ),
                    ...
                    'qty_on_regular_price_in_pro'      => esc_html__( 'Regular Price 🔒', 'min-and-max-quantity-for-woocommerce' ),
                    ...
                    'qty_on_length_in_pro'             => esc_html__( 'Length 🔒', 'min-and-max-quantity-for-woocommerce' ),
                    'qty_on_width_in_pro'              => esc_html__( 'Width 🔒', 'min-and-max-quantity-for-woocommerce' ),
                    'qty_on_height_in_pro'             => esc_html__( 'Height 🔒', 'min-and-max-quantity-for-woocommerce' ),
                    'qty_on_weight_in_pro'             => esc_html__( 'Weight 🔒', 'min-and-max-quantity-for-woocommerce' ),
         ...
         * @since 2.1.1
         */
        public function mmqw_load_categories_ajax() {
            check_ajax_referer( 'mmqw_nonce', 'security' );
            // Check if search term is set
            $search_term = '';
            if ( isset( $_GET['q'] ) ) {
            ...
            if ( !empty( $categories ) && !is_wp_error( $categories ) ) {
                foreach ( $categories as $category ) {
                    ...
                    // Build hierarchical path if category has parent
                    if ( $category->parent > 0 ) {
                        $parent_ids = get_ancestors( $category->term_id, 'product_cat' );
                        $parent_names = array();
                        ...
                        // Reverse to get top-to-bottom hierarchy
                        $parent_names = array_reverse( $parent_names );
                        $parent_names[] = $category->name;
                        $category_text = '#' . implode( '->', $parent_names );
                    }
                    $results[] = array(
                        'id'   => $category->term_id,
                        'text' => $category_text,
        ...
        public function mmqw_add_new_group_html_ajax() {
            ...
            ?>
																					...
																					<option value="length_in_pro">
																						<?php 
            esc_html_e( 'Length 🔒', 'min-and-max-quantity-for-woocommerce' );
            ?>
																					</option>
																					<option value="width_in_pro">
																						<?php 
            esc_html_e( 'Width 🔒', 'min-and-max-quantity-for-woocommerce' );
            ?>
																					</option>
																					<option value="height_in_pro">
																						<?php 
            esc_html_e( 'Height 🔒', 'min-and-max-quantity-for-woocommerce' );
            ?>
																					</option>
																					<option value="weight_in_pro">
																						<?php 
            esc_html_e( 'Weight 🔒', 'min-and-max-quantity-for-woocommerce' );
            ?>
																					</option>
																				</optgroup>
																	<?php 
                ...
                foreach ( $get_all_sm as $rule ) {
                    ...
                    if ( !empty( $sitepress ) ) {
                        if ( version_compare( ICL_SITEPRESS_VERSION, '3.2', '>=' ) ) {
                            ...
                            $language_information = ( function_exists( 'wpml_get_language_information' ) ? wpml_get_language_information( $sm_post_id ) : array() );
                        }
